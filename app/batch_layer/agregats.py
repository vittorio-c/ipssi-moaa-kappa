...
from app.batch_layer.base_methods import all_data as all_stations
from app.batch_layer.base_methods import mariadb_container_ip
...
# %%
# Par mois
mean_tmp_by_month = all_stations.groupBy([year("DATE").alias("year"), month("DATE").alias("month")]).agg(
...
# %%
# Par année
min_max_tmp_by_year = all_stations.groupBy([year("DATE").alias("year")]).agg(F.min("temperature").alias("min_tmp"),
                                                                             F.max("temperature").alias("max_tmp"))
min_max_tmp_by_year = min_max_tmp_by_year.sort("year")
min_max_tmp_by_year.head(5)

# %%
# Par mois
min_max_tmp_by_month = all_stations.groupBy([year("DATE").alias("year"), month("DATE").alias("month")]).agg(
...
# %%
# Par jour
min_max_tmp_by_day = all_stations.groupBy(to_date("DATE").cast("date").alias("date")).agg(
    F.min("temperature").alias("min_tmp"), F.max("temperature").alias("max_tmp"))
...
# %%
# Par saisons
min_max_tmp_by_season = all_stations.groupBy([year("DATE").alias("year"), "season"]).agg(
    F.min("temperature").alias("min_tmp"), F.max("temperature").alias("max_tmp"))
...
min_max_tmp_by_month_and_city = all_stations.groupBy(
    [col("NAME").alias("ville"), col("STATION"), year("DATE").alias("year"), month("DATE").alias("month")]).agg(
    F.min("temperature").alias("min_tmp"), F.max("temperature").alias("max_tmp"))
...
# %%
all_stations_with_elev = all_stations.na.drop(how="any", subset=["ELEVATION"]).filter(
    ~all_stations.ELEVATION.contains("+9999"))

mean_tmp_elevation_by_year = all_stations_with_elev.groupBy([year("DATE").alias("year"), "STATION"]).agg(
    mean("temperature").alias("mean_tmp"), mean("ELEVATION").alias("mean_elevation"))
...
# Par année
mean_dew_by_year = all_stations_with_dew.groupBy([year("DATE").alias("year")]).agg(
    mean("precipitation").alias("mean_dew"))
...
# Par mois
mean_dew_by_month = all_stations_with_dew.groupBy([year("DATE").alias("year"), month("DATE").alias("month")]).agg(
    mean("precipitation").alias("mean_dew"))
...
# Par jour
mean_dew_by_day = all_stations_with_dew.groupBy(to_date("DATE").cast("date").alias("date")).agg(
    mean("precipitation").alias("mean_dew"))
...
# Par saison
mean_dew_by_season = all_stations_with_dew.groupBy([year("DATE").alias("year"), "season"]).agg(
    mean("precipitation").alias("mean_dew"))
...
mean_dew_tmp_by_month = all_stations_with_dew.groupBy([year("DATE").alias("year"), month("DATE").alias("month")]).agg(
    mean("temperature").alias("mean_tmp"), mean("precipitation").alias("mean_dew"))
